...
# Set only for an HTTP streaming request. Normal CLI and non-streaming calls
# keep the existing invoke path and do not pay for token-level callbacks.
_speech_delta_emitter: ContextVar[Callable[[str, str], None] | None] = ContextVar(
    "speech_delta_emitter", default=None
)


def set_speech_delta_emitter(emitter: Callable[[str, str], None] | None):
    return _speech_delta_emitter.set(emitter)


def reset_speech_delta_emitter(token) -> None:
    _speech_delta_emitter.reset(token)
...
def _invoke_final_agent(agent, human: HumanMessage, node: str) -> tuple[str, list]:
    """Invoke the final-answer agent, streaming visible text when requested."""
    emitter = _speech_delta_emitter.get()
    if emitter is None:
        result = agent.invoke({"messages": [human]})
        return _message_content_text(result["messages"][-1].content).strip(), [human, result["messages"][-1]]

    chunks: list[str] = []
    try:
        for item in agent.stream({"messages": [human]}, stream_mode="messages"):
            if not isinstance(item, tuple) or len(item) != 2:
                continue
            message_chunk, metadata = item
            if isinstance(metadata, dict) and metadata.get("langgraph_node") not in {None, "model"}:
                continue
            text = _message_content_text(getattr(message_chunk, "content", message_chunk))
            if text:
                chunks.append(text)
                emitter(node, text)
    except Exception:
        # If the provider failed before producing output, preserve the old
        # invoke behavior so a transient streaming capability issue is safe.
        if not chunks:
            result = agent.invoke({"messages": [human]})
            return _message_content_text(result["messages"][-1].content).strip(), [human, result["messages"][-1]]
    content = "".join(chunks).strip()
    return content, [human, AIMessage(content=content)]
...
def direct_answer_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    human = HumanMessage(content=with_memory_context(state, prompt))
    content, messages = _invoke_final_agent(agent, human, "direct_answer")
...
def write_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    content, messages = _invoke_final_agent(agent, human, "write")
    ...
    return {"draft": final_content, "final": final_content, "messages": messages}
