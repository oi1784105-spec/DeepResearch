...
def build_agent(
    ...
    if provider == "openai_compatible":
        ...
        llm = ChatOpenAI(
            ...
            temperature=None if reasoning_effort != "auto" else temperature,
            reasoning_effort=None if reasoning_effort == "auto" else reasoning_effort,
        )
    elif provider == "anthropic":
        from langchain_anthropic import ChatAnthropic

        llm = ChatAnthropic(model=model, api_key=api_key, temperature=temperature, base_url=base_url or None)
...
def build_agents(model: str, api_key: str, config: AppConfig) -> AgentBundle:
    ...
    def create_configured_agent(prompt_key: str, temperature: float):
        return build_agent(
            model=model,
            api_key=api_key,
            ...
            reasoning_effort=getattr(config, "reasoning_effort", "auto"),
        )

    # 去掉每个 Agent 强制绑定的 tools，只做信息抽取，降低 System Prompt 长度
    return AgentBundle(
        intent_router=create_configured_agent("intent_router", 0.0),
        planner=create_configured_agent("plan", 0.3),
        scout_web=create_configured_agent("web_search", 0.4),
        scout_local=create_configured_agent("local_rag", 0.4),
        evidence_judge=create_configured_agent("deep_dive", 0.2),
        analyst=create_configured_agent("analyze", 0.3),
        direct_responder=create_configured_agent("direct_answer", 0.2),
        writer=create_configured_agent("write", 0.4),
    )
