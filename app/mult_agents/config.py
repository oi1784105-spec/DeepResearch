...
class AppConfig:
    ...
    def _resolve_api_key(data: dict) -> str:
        for env_key in ("LLM_API_KEY", "DASHSCOPE_API_KEY", "OPENAI_API_KEY"):
    ...
    @staticmethod
    def from_file(path: str | Path | None = None, require_api_key: bool = True) -> "AppConfig":
        ...
        llm_provider = AppConfig._resolve_str(data, "llm_provider", "LLM_PROVIDER", "dashscope").lower()
        if llm_provider not in {"dashscope", "openai_compatible"}:
            raise ValueError("LLM_PROVIDER 仅支持 dashscope 或 openai_compatible")
        api_key = AppConfig._resolve_api_key(data)
        if require_api_key and not api_key:
            raise ValueError(
                f"缺少 LLM_API_KEY 配置，请在 {config_path} 中填写 api_key，或设置环境变量 LLM_API_KEY"
            )
        base_url = AppConfig._resolve_str(data, "base_url", "LLM_BASE_URL", "")
        if llm_provider == "openai_compatible" and not base_url:
            raise ValueError("使用 openai_compatible 时必须配置 LLM_BASE_URL")
        model = AppConfig._resolve_str(data, "model", "MODEL", "qwen-plus")
        reasoning_effort = AppConfig._resolve_str(data, "reasoning_effort", "REASONING_EFFORT", "auto").lower()
